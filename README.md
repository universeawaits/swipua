...
- **🇨🇳 Chinese (Simplified)** — 2,000+ words and a 24-topic grammar reference (pinyin & the tone system, how hanzi are built from radicals, word order, measure words, the aspect particles 了/着/过, the complement system, 把 and 被 sentences, 是…的…), studied from Russian, German, English, Turkish, Italian, or Argentine Spanish. **Every word and every example sentence carries pinyin**, so you can read from day one. The levels are labelled on the **HSK** ladder (HSK 1–2 / HSK 3 / HSK 4) rather than CEFR, because that is where A1–B1 actually ends — HSK 5–6 is already B2/C1.
- **🇯🇵 Japanese** — 3,500+ words and a 24-topic grammar reference (the writing system, は vs が, the particle set, the three verb groups, the て-form and plain form, giving-and-receiving, noun-modifying clauses, the four conditionals, transitive/intransitive pairs, the passive and causative, **keigo**, evidentiality, nominalisation, aspect), studied from Russian, German, English, Turkish, Italian, or Argentine Spanish. **Every word and every example carries Hepburn romaji**, so the kanji-kana mix is readable from the start. Levels are labelled on the **JLPT** ladder (N5 / N4 / N3).
- **🇰🇷 Korean** — 4,000+ words and a 24-topic grammar reference (Hangeul and its pronunciation rules, the particle set, the four **speech levels**, conjugation and the seven irregular stem classes, **honorifics**, noun-modifying endings, the auxiliary verbs, indirect quotation, the 이/히/리/기 passive-causative, evidential guessing, nominalisation, discourse endings), studied from Russian, German, English, Turkish, Italian, or Argentine Spanish. **Every word and every example carries Revised Romanisation.** Levels are labelled on the **TOPIK** ladder (TOPIK 1 / 2 / 3).
- **🇹🇭 Thai** — 2,300+ words and a 24-topic grammar reference (the script and its three consonant classes, the tone rule, classifiers, เป็น/คือ/อยู่/มี, aspect markers in place of tense, the four ways to say “can”, serial verbs, the ถูก/โดน adversative passive, sentence-final particles, registers, ใจ-compounds), studied from English, German, Russian, French, or Persian. Thai script is unspaced and tonal, so **every word and every example carries a learner-phonetic transcription with tone marks** (Paiboon-style, not RTGS). Plain CEFR levels (A1/A2/B1), CU-TFL / CEFR-aligned.
...
- **Real vocabulary with example sentences** — 4,600+ words for German, 4,000+ each for Spanish, French, Ukrainian, Croatian, Belarusian and Korean, 3,700+ for Turkish, 3,500+ for Japanese, 3,300+ each for Russian and Italian, 2,300+ for Thai, 2,000+ for Chinese.
...
- **Hear it spoken** — a speaker button next to the word you're learning and each example plays its pronunciation in the target language. Every clip is **pre-rendered with [Piper](https://github.com/rhasspy/piper)**, an open-source neural voice, so playback is **instant and natural** — no synthesis lag, no dependence on the voices your device happens to have. The clips live in a separate [`beeins-audio`](https://github.com/universeawaits/beeins-audio) repo and load from the jsDelivr CDN (edge- and browser-cached; only the tiny clips you actually play are fetched), which keeps this repo small. If a clip is ever unavailable it falls back to synthesizing in the browser (Piper via WebAssembly), then to the OS text-to-speech. Chinese, Japanese, Korean and Thai ship without pre-rendered clips and use your device's zh-CN / ja-JP / ko-KR / th-TH voice directly.
...
- **Skins** — pick a whole look from the emoji dropdown: 🌿 leaf (default), 🫧 bubbles, 🧊 ice, 💧 raindrop, 🔮 crystal, ✨ aurora, 🪩 disco, 🌼 daisy, 🍷 wine, 🩸 blooddrop, 🍕 pizza, 📜 scroll. Each restyles the accent, card shape, surface (tint, frosted glass, glow), typography (rounded, serif, monospace, typewriter…) and page backdrop — not just the accent colour — and works in both light and dark.
